...
(Unpacked installs also accept `window.postMessage({ type: 'ff-dev-reload' }, '*')` from any supported site's console, which reloads the extension — handy for automated test loops. Reload the page afterwards.)
